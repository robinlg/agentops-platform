...
//go:generate mockgen -destination mock_store.go -package store github.com/robinlg/agentops-platform/internal/store IStore
...
var (
	once sync.Once
	// 全局变量，方便其它包直接调用初始化好的 datastore 实例
	S *datastore
)
...
// AgentRun 返回一个实现了 AgentRunStore 接口的实例
func (store *datastore) AgentRun() AgentRunStore {
	return newAgentRunStore(store)
}

// Conversation 返回一个实现了 ConversationStore 接口的实例
func (store *datastore) Conversation() ConversationStore {
	return newConversationStore(store)
}
